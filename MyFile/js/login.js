...
function render() {
	...
	if (intersects.length > 0) {
		(function () {
			...
			intersects[0].face.color.setStyle("#00FFFF");
			plane.geometry.colorsNeedUpdate = true;
		})();
	}
    ...
    methods:{
        ...
        btn:function () {
            ...
            if(this.name === "admin" && this.password === "123"){
                ...
                introTimeline.add([TweenLite.fromTo($("#login"), 1, { opacity: 1 }, { opacity: 0, ease: Power3.easeIn }), TweenLite.to(camera.rotation, 3, { x: Math.PI / 2, ease: Power3.easeInOut }), TweenLite.to(camera.position, 2.5, { z: 20, ease: Power3.easeInOut }), TweenLite.to(camera.position, 3, { y: 120, ease: Power3.easeInOut }), TweenLite.to(plane.scale, 3, { x: 2, ease: Power3.easeInOut })]);
                introTimeline.add([TweenLite.to($(".x-mark"), 3, { opacity: 1, ease: Power3.easeInOut })],[TweenLite.to($(".container"), 5, { opacity: 1, ease: Power3.easeInOut })]);
            }
        }
...
var cancel = new Vue({
	...
	methods:{
        cancel:function () {
            ...
            outroTimeline.add([TweenLite.to($(".x-mark"), 0.5, { opacity: 0, ease: Power3.easeInOut }),TweenLite.to(camera.rotation, 3, { x: 0, ease: Power3.easeInOut }), TweenLite.to(camera.position, 3, { z: 50, ease: Power3.easeInOut }), TweenLite.to(camera.position, 2.5, { y: 0, ease: Power3.easeInOut }), TweenLite.to(plane.scale, 3, { x: 1, ease: Power3.easeInOut })]);
            outroTimeline.add([TweenLite.to($("#login"), 0.5, { opacity: 1, ease: Power3.easeIn })],[TweenLite.to($(".container"), 1, { opacity: 0, ease: Power3.easeInOut })]);
        }
	}
});